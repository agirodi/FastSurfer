...
class H5pyDataset:
    ...
    def _load_volumes(self, subject_path):
        ...
        Args:
            subject_path (str): path to subjectfile

        Returns:
            np.ndarray: original image
            np.ndarray: segmentation ground truth
            np.ndarray: segmentation ground truth without corpus callosum
            tuple: tuple zoom values
        """
    ...
    def transform(self, plane, imgs, zoom):
        ...
        Args:
            plane (str): plane (sagittal, axial, )
            imgs (np.ndarray): input image
            zoom (np.ndarray): zoom factors

        Returns:
            np.ndarray: transformed image
            int: transformed zoom facors
    ...
    def _pad_image(self, img, max_out):
        ...
        Args:
            ...
            max_out (int): size of output image

        Returns:
            np.ndarray: 0-padded image to the given size
        """
    ...
    def create_hdf5_dataset(self, plane='axial'):
        """
        creates a hdf5 file with all given data

        Args:
            plane (str): plane (sagittal, axial, coronal)
        """
